...
function DocumentConversion(options, comments) {
  ...
  function htmlPathForParagraph(element, messages) {
    ...
    if (element.outlineLvl) {
      const h = styleMap.filter(x => x.from._styleId === `Heading${element.outlineLvl}`)[0];
      if (h) {
        return clone(h).to;
      } else {
        console.log(`Outline level too deep (level ${element.outlineLvl})`);
        // console.log(style);
        // console.log(styleMap[0]);
        // throw new Error('OMG');
      }
    }
    if (style) {
